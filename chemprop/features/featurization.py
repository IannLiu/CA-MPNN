...
def atom_features(atom: Chem.rdchem.Atom, functional_groups: List[int] = None) -> List[Union[bool, int, float]]:
    ...
    if atom is None:
        ...
        features = onek_encoding_unk(atom.GetAtomicNum() - 1, PARAMS.ATOM_FEATURES['atomic_num']) + \
            ...
            onek_encoding_unk(atom.GetNumRadicalElectrons(), PARAMS.ATOM_FEATURES['radical']) + \
        ...
        if not self.is_reaction:
            ...
            for a1 in range(self.n_atoms):
                for a2 in range(a1 + 1, self.n_atoms):
                    if sort_by_atom_map:
                        idx1 = atoms[a1].GetIdx()
                        idx2 = atoms[a2].GetIdx()
                    ...
                    b1 = self.n_bonds        # bond index, b -> a1
                    b2 = b1 + 1              # reverse bond index, b -> a2
                    self.a2b[a2].append(b1)  # b1 = a1 --> a2
                    self.b2a.append(a1)
                    self.a2b[a1].append(b2)  # b2 = a2 --> a1
                    self.b2a.append(a2)
                    self.b2revb.append(b2)   # b1 -> b2
                    self.b2revb.append(b1)   # b2 -> b1
...
class ReactionGraph:
    """
    A :class:`MolGraph` represents the graph structure and featurization of a reaction.
    """

    def __init__(self, mol: Union[str, Tuple[Chem.Mol, Chem.Mol]]):
        """
        :param mol: A SMILES or an RDKit molecule.
        """
        self.is_mol = is_mol(mol)
        ...
        self.n_atoms = 0  # number of atoms
        self.n_bonds = 0  # number of bonds
        self.rf_atoms = []  # mapping from atom index to atom features
        self.rf_bonds = []  # mapping from bond index to concat(in_atom, bond) features
        self.pf_atoms = []
        self.pf_bonds = []
        self.a2b = []  # mapping from atom index to incoming bond indices
        self.b2a = []  # mapping from bond index to the index of the atom the bond is coming from
        ...
        # Get atom features, here we just consider the balance atoms case
        # That is, the atom in reactant must appear in the product
        ratoms = sorted(mol_reac.GetAtoms(), key=lambda a: a.GetAtomMapNum())
        patoms = sorted(mol_prod.GetAtoms(), key=lambda a: a.GetAtomMapNum())
        self.rf_atoms = [atom_features(atom) for atom in ratoms]
        self.pf_atoms = [atom_features(atom) for atom in patoms]

        self.n_atoms = len(ratoms)

        for ra, pa in zip(ratoms, patoms):
            assert ra.GetSymbol() == pa.GetSymbol(), 'reactant atom and product atom cannot be aligned'
        ...
        # Initialize f_bonds to real bonds mapping for each bond
        self.b2br = np.zeros([len(mol_reac.GetBonds()), 2])

        # Get bond features
        for a1 in range(self.n_atoms):
            for a2 in range(a1 + 1, self.n_atoms):
                ridx1 = ratoms[a1].GetIdx()
                ridx2 = ratoms[a2].GetIdx()
                pidx1 = patoms[a1].GetIdx()
                pidx2 = patoms[a2].GetIdx()
                bond_reac = mol_reac.GetBondBetweenAtoms(ridx1, ridx2)
                bond_prod = mol_prod.GetBondBetweenAtoms(pidx1, pidx2)

                if bond_reac is None and bond_prod is None:
                    continue

                f_bond_reac = bond_features(bond_reac)  # reactant atom and bond feature  must equal to that of product
                f_bond_prod = bond_features(bond_prod)  #

                self.rf_bonds.append(self.rf_atoms[a1] + f_bond_reac)
                self.rf_bonds.append(self.rf_atoms[a2] + f_bond_reac)

                self.pf_bonds.append(self.pf_atoms[a1] + f_bond_prod)
                self.pf_bonds.append(self.pf_atoms[a2] + f_bond_prod)
...
class BatchReactionGraph:
    """

    """

    def __init__(self, mol_graphs: List[ReactionGraph]):
        r"""
        :param mol_graphs: A list of :class:`MolGraph`\ s from which to construct the :class:`BatchMolGraph`.
        """
        self.atom_fdim = get_atom_fdim(is_reaction=False)
        self.bond_fdim = get_bond_fdim(is_reaction=False)
        ...
        # All start with zero padding so that indexing with zero padding returns zeros
        rf_atoms = [[0] * self.atom_fdim]  # atom features
        rf_bonds = [[0] * self.bond_fdim]  # combined atom/bond features

        pf_atoms = [[0] * self.atom_fdim]  # atom features
        pf_bonds = [[0] * self.bond_fdim]  # combined atom/bond features
        a2b = [[]]  # mapping from atom index to incoming bond indices
        b2a = [0]  # mapping from bond index to the index of the atom the bond is coming from
        b2revb = [0]  # mapping from bond index to the index of the reverse bond
        for mol_graph in mol_graphs:
            rf_atoms.extend(mol_graph.rf_atoms)
            rf_bonds.extend(mol_graph.rf_bonds)

            pf_atoms.extend(mol_graph.pf_atoms)
            pf_bonds.extend(mol_graph.pf_bonds)
        ...
        self.rf_atoms = torch.tensor(rf_atoms, dtype=torch.float)
        self.rf_bonds = torch.tensor(rf_bonds, dtype=torch.float)
        self.pf_atoms = torch.tensor(pf_atoms, dtype=torch.float)
        self.pf_bonds = torch.tensor(pf_bonds, dtype=torch.float)
        self.a2b = torch.tensor([a2b[a] + [0] * (self.max_num_bonds - len(a2b[a])) for a in range(self.n_atoms)], dtype=torch.long)
        self.b2a = torch.tensor(b2a, dtype=torch.long)
        self.b2revb = torch.tensor(b2revb, dtype=torch.long)
        self.b2b = None  # try to avoid computing b2b b/c O(n_atoms^3)
    ...
    def get_components(self, atom_messages: bool = False) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor,
                                                                   torch.Tensor, torch.Tensor, torch.Tensor,
                                                                   List[Tuple[int, int]], List[Tuple[int, int]]]:
        """
        Returns the components of the :class:`BatchMolGraph`.
        ...
        * :code:`b_scope`
        ...
        """
        if atom_messages:
            rf_bonds = self.rf_bonds[:, -get_bond_fdim(atom_messages=atom_messages):]
            pf_bonds = self.pf_bonds[:, -get_bond_fdim(atom_messages=atom_messages):]
        else:
            rf_bonds = self.rf_bonds
            pf_bonds = self.pf_bonds

        return self.rf_atoms, self.pf_atoms, rf_bonds, pf_bonds, self.a2b, self.b2a, self.b2revb, self.a_scope, self.b_scope
...
def mol2graph(mols: Union[List[str], List[Chem.Mol], List[Tuple[Chem.Mol, Chem.Mol]]],
              ...
              overwrite_default_atom_features: bool = False,
              overwrite_default_bond_features: bool = False,
              individual_react_prod_mol: bool = False) -> Union[BatchMolGraph, BatchReactionGraph]:
    """
    ...
    :param individual_react_prod_mol: use reaction batch rather than molecule batch
    :return: A :class:`BatchMolGraph` containing the combined molecular graph for the molecules.
    """
    if individual_react_prod_mol and type(mols[0]) == tuple:
        return BatchReactionGraph([ReactionGraph(mol)for mol in mols])
    else:
